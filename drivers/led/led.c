...
void led_init(unsigned int led, enum gpio_set original_state)
{
    switch (led) {
        case LED_RED:
        {
            gpio_init(LED_RED_PORT, LED_RED_PIN, OUTPUT, PUSH, GPIO_2MHZ, UP_PULL, original_state);
            break;
        }
        case LED_GREEN:
        {
            gpio_init(LED_GREEN_PORT, LED_GREEN_PIN, OUTPUT, PUSH, GPIO_2MHZ, UP_PULL, original_state);
            break;
        }
        default:
        break;
