...
static void main_pll_cfg(enum clock_pll_presc pllq, enum clock_pll_presc pllp,
			 enum clock_pll_presc plln, enum clock_pll_presc pllm)
{
	RCC_PLLCFGR &= ~(0xf << 24);
	RCC_PLLCFGR |= (pllq << 24);
...
/* If the clock source is high speed external clock */
	...
	if (clk_int.sysclk_src == PLL) {
		...
		main_pll_cfg(clk_int.mpll_q, clk_int.mpll_p, clk_int.mpll_n,
			     clk_int.mpll_m);

		/* export system parameters */
		_sysclk_frq =
			(((HSE_CLK_SRC / clk_int.mpll_m) * clk_int.mpll_n) / 2);
