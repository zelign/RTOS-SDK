...
static inline void set_baud_rate(unsigned int baud)
{
	...
	unsigned int div_f =
		(unsigned char)(((((double)_apb2_frq * 1000000) /
				  (/*(double)*/ (baud * 8 * (2 - over8)))) -
				 div_m) *
				16);
	USART1_BRR &= ~(0xffff);
	USART1_BRR |= (div_m << 4);
...
static inline void usart_pin_ctl(enum gpio_reg gpio, enum gpio_pin pin,
				 enum gpio_afr afr, enum gpio_moder moder)
{
	gpio_port_set(gpio, pin, moder, PUSH, GPIO_50MHZ, UP_PULL);
}

static inline void usart1_reuse_ctl(enum gpio_reg gpio, enum gpio_pin pin,
				    enum gpio_afr afr, enum gpio_moder moder)
{
