		   ...
		   enum gpio_uppull pull)
{
	...
	if (moder != -1) {
		*((uint32_t volatile *)(GPIO_BASE_ADDR + GPIO + MODER)) &=
			~(0x3 << (pin * 2));
		*((uint32_t volatile *)(GPIO_BASE_ADDR + GPIO + MODER)) |=
			(moder << (pin * 2));
	}
	...
	if (oty != -1) {
		...
		*((uint32_t volatile *)(GPIO_BASE_ADDR + GPIO + OTYPER)) &=
			~(1 << pin);
		*((uint32_t volatile *)(GPIO_BASE_ADDR + GPIO + OTYPER)) |=
			(oty << pin);
	}
	...
	if (speed != -1) {
		...
		*((uint32_t volatile *)(GPIO_BASE_ADDR + GPIO + OSPEEDER)) &=
			~(0x3 << pin * 2);
		*((uint32_t volatile *)(GPIO_BASE_ADDR + GPIO + OSPEEDER)) |=
			(speed << pin * 2);
	}
	...
	if (pull != -1) {
		...
		*((uint32_t volatile *)(GPIO_BASE_ADDR + GPIO + PUPDR)) &=
			~(0x3 << pin * 2);
		*((uint32_t volatile *)(GPIO_BASE_ADDR + GPIO + PUPDR)) |=
			(pull << pin * 2);
	}
...
{
	if (gset)
		*((uint32_t volatile *)(GPIO_BASE_ADDR + GPIO + BSRR)) |=
			(1 << pin);
	else
		*((uint32_t volatile *)(GPIO_BASE_ADDR + GPIO + BSRR)) |=
			(1 << (16 + pin));
...
{
	return (((*((uint32_t volatile *)(GPIO_BASE_ADDR + GPIO + IDR))) >>
		 pin) &
		0x01);
}
...
{
	if (pin < GPIO_PIN_8) {
		*((volatile unsigned int *)(GPIO_BASE_ADDR + gpio + AFRL)) &=
			~(0xf << (pin * 4));
		*((volatile unsigned int *)(GPIO_BASE_ADDR + gpio + AFRL)) |=
			(afr << (pin * 4));
	} else {
		*((volatile unsigned int *)(GPIO_BASE_ADDR + gpio + AFRH)) &=
			~(0xf << ((pin - 8) * 4));
		*((volatile unsigned int *)(GPIO_BASE_ADDR + gpio + AFRH)) |=
			(afr << ((pin - 8) * 4));
	}
