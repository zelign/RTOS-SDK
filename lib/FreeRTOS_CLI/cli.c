...
void cli_executor_loop(void *para)
{
	...
	for (;;) {
		...
		if (uart_readable_flag) {
			if (cli_read == '\n') {
				if (cli_read_buf_p >
				    &cli_read_buf[0]) { //confirm that the command only has '\n'
					*cli_read_buf_p =
						'\0'; //add the terminal for this command
						...
						if ((!strncmp(
							...
							FreeRTOS_CLIProcessCommand(
								...
								&cli_read_buf[strlen(
									pxCommand
										->pxCommandLineDefinition
										->pcCommand)], /* subcommand */
								strlen(&cli_read_buf[strlen(
									pxCommand
										->pxCommandLineDefinition
										->pcCommand)])); //subcommand length
							memset(&cli_read_buf[0],
							       0, 64);
							cli_read_buf_p =
								&cli_read_buf[0];
							break;
					...
					if (!pxCommand) {
						...
						putc_usart1('\n');
						printf(GREEN_LOG,
						       STM32F407ZGT6_PREFIX);
						printf(RED_LOG,
						       PREFIX_SEPARATOR);
						printf(LIGHT_BLUE_LOG,
						       STM32F407ZGT6_SUFFIX);
						printf(LIGHT_BLUE_LOG,
						       SUFFIX_SEPARATOR);
...
{
	xTaskCreate(cli_executor_loop, "cli executor",
