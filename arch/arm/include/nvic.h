...
#define NVIC_ENABLE(x)                                            \
	do {                                                      \
		*((volatile unsigned int *)(ISER_BASE_ADDR +      \
					    (((x) / 32) * 4))) |= \
			(0x1 << ((x) % 32));                      \
	} while (0);
...
#define NVIC_DISABLE(x)                                           \
	do {                                                      \
		*((volatile unsigned int *)(ICER_BASE_ADDR +      \
					    (((x) / 32) * 4))) &= \
			~(0x1 << ((x) % 32));                     \
	} while (0);
...
void set_priority(enum priority_group prio_g, unsigned char empt_prio,
		  unsigned char sub_priority, enum EXC_NUM exc_num);
