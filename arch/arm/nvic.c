...
{
    return ((*((volatile unsigned int *)(ISPR_BASE_ADDR + ((exc_num / 32) * 4))) &
                                        ((0x1 << ((exc_num % 32) - 1)))) ? 1 : 0);
}
...
{
    return ((*((volatile unsigned int *)(IABR_BASE_ADDR + ((exc_num / 32) * 4))) &
                                        ((0x1 << ((exc_num % 32) - 1)))) ? 1 : 0);
}
...
{
    *((volatile unsigned int *)(IP_BASE_ADDR + (exc_num / 4) * 4)) &= ~(0xFF << (exc_num % 4) * 8);
    *((volatile unsigned int *)(IP_BASE_ADDR + (exc_num / 4) * 4)) |= (prio << (exc_num % 4) * 8);
}

void set_priority(enum priority_group prio_grp,
